...
class AutoModelForCausalLMWithCountValueHead(AutoModelForCausalLMWithValueHead):
    ...
    def forward(
        ...
        **model_kwargs,
        ...
        value_output = self.value_head(last_hidden_state.detach())                               # TODO: 不更新 base model
        # print("in AutoModelForCausalLMWithCountValueHead: last_hidden_state:", last_hidden_state.shape)      # (batch_size, seq_length, 4096), e.g.,torch.Size([8, 539, 4096])
        # print("in AutoModelForCausalLMWithCountValueHead: value_output:", value_output.shape)                # (batch_size, 20), e.g.,torch.Size([8, 20])
...
class CountTrainer(Trainer):
    ...
    def optimistic_count_loss(self, value_output: torch.FloatTensor) -> torch.FloatTensor:
        ...
        # print("In optimistic_count_loss: value_output:", value_output.shape, ", coin_flip_target:", coin_flip_target.shape)   # value_output: torch.Size([8, 20]) , coin_flip_target: torch.Size([8, 20])
...
if __name__ == "__main__":    
    ...
    # model_args.load_in_8bit = True                              # TODO: load in 8 bit 单卡运行需求
