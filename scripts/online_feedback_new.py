...
reward_model = AutoModelForSequenceClassification.from_pretrained(reward_model_name, device_map="cuda", trust_remote_code=True, torch_dtype=torch.bfloat16)
...
def generate_response_vllm(model, tokenizer, dataset):
    with torch.inference_mode():
        ...
        scores = []
        print("\n\nlength:", len(prompts))
        print("candidates_texts[0]:", len(candidates_texts[0]))
        for i in range(len(prompts)):
            score_1 = candidates_texts[i][0].score.float().item()
            score_2 = candidates_texts[i][1].score.float().item()
            score_3 = candidates_texts[i][2].score.float().item()
            print("scores:", score_1, score_2, score_3)
            scores.append([score_1, score_2, score_3])
        scores = np.array(scores)

        chosen_indices = np.argmax(scores, axis=1)                                            # 找到最好的回复 (2000,)  # 注意这里修改了min/max
        rejected_indices = np.argmix(scores, axis=1)                                          # 找到最差的回复 (2000,)
