...
class SELMCountTrainer(DPOTrainer):
    def __init__(
        self,
    ...
    # TODO: 计算 count 导致的内在激励
    def count_reward(self, last_hidden_state):
        # with torch.no_grad():
        # print("device1:", self.value_head.summary.weight.device, ", device2:", last_hidden_state.device)
        ...
        flips = self.value_head(last_hidden_state)                                     # TODO: 不更新 base model
        # print("\n\nIn count_reward: last_hidden_state:", last_hidden_state.shape)    # e.g., ([1, 456, 4096])
        # print("In count_reward: flips:", flips.shape)                                # e.g., [1, 20]
        
        mean_square_flips = torch.mean(torch.square(flips), dim=1, keepdim=True)
        # print("In count_reward: mean_square_flips:", mean_square_flips.shape, last_hidden_state.shape)  # torch.Size([1, 1]) torch.Size([1, 668, 4096])
    ...
    def optimistic_dpo_loss(
        ...
        if self.loss_type == "sigmoid":      
            ...
            losses = -F.logsigmoid(self.beta * logits) - count_reward               # TODO: count_reward * -1.0
        ...
        return losses, chosen_rewards, rejected_rewards, count_reward
    ...
    def get_batch_loss_metrics(
        ...
        losses, chosen_rewards, rejected_rewards, count_reward = self.optimistic_dpo_loss(     # TODO: 增加了count-reward 为了log
            policy_chosen_logps,
            policy_rejected_logps,
            policy_response_logps,
            reference_chosen_logps,
        ...
        metrics[f"{prefix}rewards/count_reward"] = count_reward.detach().mean().cpu()      # TODO: log count-reward
